...
| `metrics_order` | `list` | | _auto_ | Which metric tiles to draw, and in which order, dragged into place in the visual editor. Direct level: `remaining`, `average`, `consumption`, `autonomy`. Pump runtime: `remaining`, `today`, `pump7d`. **An entity id draws that entity instead**, rendered by Home Assistant itself, so a text state arrives translated; the tile opens the entity when tapped, and keeps its place showing `—` when the sensor drops. **What is left out is hidden**, an empty list drops the row entirely, and anything past the fourth tile is ignored: three sit in a row and four fall into a 2x2, five would have to shrink. Omit the option and the card keeps the arrangement it has always drawn |
| `window` | `number` | ² | `7` | `7`, `28` or `84` days, offered as 1, 4 and 12 weeks. Always seven bars, of one, four or twelve days each. It drives the chart, the consumption tile and the autonomy together. **Your recorder must hold that much history**: it purges at 10 days by default, and the card then falls back to the span it really covered, says so under the chart, and divides by that instead of by the window |
