...
| `metrics_order` | `list` | | _auto_ | Which metric tiles to draw, and in which order, dragged into place in the visual editor. Direct level: `remaining`, `average`, `consumption`, `autonomy`. Pump runtime: `remaining`, `today`, `pump7d`. **What is left out is hidden**, and an empty list drops the row entirely. Omit the option and the card keeps the arrangement it has always drawn |
