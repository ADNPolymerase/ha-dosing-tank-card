...
| `metrics_order` | `list` | | _auto_ | Quelles tuiles dessiner, et dans quel ordre, à glisser dans l'éditeur visuel. Niveau direct : `remaining`, `average`, `consumption`, `autonomy`. Temps de pompe : `remaining`, `today`, `pump7d`. **Un identifiant d'entité affiche cette entité**, rendue par Home Assistant lui-même, donc un état textuel arrive traduit ; la tuile ouvre la fiche au clic, et garde sa place avec un `—` si le capteur décroche. **Ce qui n'y figure pas est masqué**, une liste vide supprime la rangée, et tout ce qui dépasse la quatrième tuile est ignoré : trois tiennent sur une ligne, quatre passent en 2×2, cinq devraient rétrécir. Sans cette option, la carte garde la disposition qu'elle a toujours eue |
| `window` | `number` | ² | `7` | `7`, `28` ou `84` jours, proposés en 1, 4 et 12 semaines. Toujours sept barres, de un, quatre ou douze jours. Elle pilote ensemble le graphe, la tuile de consommation et l'autonomie. **Encore faut-il que votre recorder garde autant d'historique** : il purge à 10 jours par défaut, et la carte retombe alors sur la durée réellement couverte, le signale sous le graphe, et divise par elle plutôt que par la fenêtre |
