...
| `metrics_order` | `list` | | _auto_ | Quelles tuiles dessiner, et dans quel ordre, à glisser dans l'éditeur visuel. Niveau direct : `remaining`, `average`, `consumption`, `autonomy`. Temps de pompe : `remaining`, `today`, `pump7d`. **Ce qui n'y figure pas est masqué**, et une liste vide supprime la rangée. Sans cette option, la carte garde la disposition qu'elle a toujours eue |
